...
export const ProductPriceChart = ({
  ...
  const renderCustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div
          className={`p-3 rounded-md shadow-lg ${theme === "dark" ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}
        >
          <p className="text-sm font-medium">
            {new Date(label).toLocaleDateString()}
          </p>
          <p
            className={`text-base font-bold ${theme === "dark" ? "text-blue-400" : "text-blue-600"}`}
          >
  ...
  return (
    ...
    >
      ...
      >
              ...
              <ResponsiveContainer width="100%" height="100%">
                {chartType === "line" ? (
                  ...
                  >
                    ...
                    {priceData.length > 10 && (
                      <Brush dataKey="date" height={20} stroke="#1a73e8" />
                    )}
                  </LineChart>
                ) : chartType === "bar" ? (
                  <BarChart
                ...
                <Table>
                  ...
                  <TableBody>
                    {statistics.sortedByDate.map((data, index) => {
                      const prevPrice =
                        index < statistics.sortedByDate.length - 1
                          ? statistics.sortedByDate[index + 1].price
                          : data.price;
        ...
        <CardFooter
          className={`grid grid-cols-3 gap-2 text-sm border-t ${theme === "dark" ? "border-gray-700" : "border-gray-200"} pt-4`}
        >
